        ...
        success:function(arr){
            ...
            $(".add_to_cart").click(function(){
                ...
                if(first){
                    ...
                    $.cookie("goods",JSON.stringify([{"id":arr[0].id,"num":arr[0].num,"size":arr[0].size,"price":arr[0].price}]),{expires:7,path:"/"})
            ...
            //循环一下goodArr, 判断该商品在cookie中是否第一次添加，是的话，在cookie中对应的商品数量信息为1，如果为多次添加改变该商品在cookie中的数量信息
                ...
                //上面循环结束后如果cookie中没有该商品信息
                    if(!isExistence){
                        goodsArr.push({"id":arr[0].id,"num":arr[0].num,"size":arr[0].size,"price":arr[0].price});
                    }
              //最后不论哪种情况，都要储存到cookie中,且注意要把goodArr数组 转成json格式
                   $.cookie("goods",JSON.stringify(goodsArr),{expires:7,path:"/"}) 
                }
